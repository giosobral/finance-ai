...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  minha vida financeira. As transações estão divididas por ponto  e vírgula. A estrutura de cada uma é: {DATA}-{TIPO}-{VALOR}-{CATEGORIA}.
  São elas: 
  ${transactions
    .map(
      (transaction) =>
